...
@st.cache_data(ttl=3600, show_spinner=False)
def load_taiwan_broker_branches(pipeline_version: str) -> pd.DataFrame:
    del pipeline_version
...
def render_lightweight_broker_branch_page() -> None:
    """Render daily, weekly, and monthly branch reports without the full backtest."""
    st.subheader("券商分點日／週／月追蹤")
    st.caption(
        "分別追蹤當日、近 5 個交易日與近 20 個交易日法人淨流入前 10 檔｜"
        "分點集中度＋法人流向＋價格確認"
    )
    ...
    broker_tickers = tuple(
        code_to_yahoo[code]
        for code in branch_trades["Ticker"].astype(str).unique()
    ...
    base_securities = pd.DataFrame()
    base_groups = pd.DataFrame()
    if not recent_prices.empty and not institutional_flows.empty:
        base_securities, base_groups = calculate_fund_flow_signals(
            recent_prices,
            institutional_flows,
            taiwan_master,
        )
    ...
    for period_tab, (horizon, settings) in zip(period_tabs, horizon_settings.items(), strict=True):
        with period_tab:
            if base_securities.empty:
                ...
                institutional_period, _ = apply_flow_horizon(
                    base_securities,
                    base_groups,
                    horizon,
                )
            ...
            if candidates.empty:
                st.info(f"{settings['label']}沒有足夠資料可建立券商分點排名。")
                continue
            ...
            if covered_stocks < 10:
                st.warning(
                    ...
                    "公開來源達匿名查詢上限後會暫停回傳，其餘資料將由每日更新繼續補齊。"
                )
            ...
            st.markdown(
                f"### {settings['label']}法人流入前 10 檔：券商分點狀況"
                + (f"（目前 {covered_stocks} 檔）" if covered_stocks < 10 else "")
            )
            st.dataframe(
                view[
                    [
            ...
            with chart_left:
                st.markdown(f"### {settings['label']}各股票最大買超分點")
                st.plotly_chart(
                    px.bar(
                    ...
                    width="stretch",
                    key=f"light_broker_buyers_{horizon}",
                )
            with chart_right:
                st.markdown(f"### {settings['label']}分點集中度 × 法人確認")
                st.plotly_chart(
                    px.scatter(
                        candidates,
                        ...
                        labels={"Branch concentration score": "分點集中分數",
                                "Selected net value": f"{settings['institutional_period']}法人估算淨流入"},
                    ).update_layout(height=520),
                    width="stretch",
                    key=f"light_broker_scatter_{horizon}",
                )
            ...
            selected_label = st.selectbox(
                f"查看{settings['label']}個股分點明細",
                list(option_map),
                key=f"light_broker_detail_{horizon}",
            )
            detail = branch_activity[
                branch_activity["Ticker"] == option_map[selected_label]
            ].copy()
...
with st.sidebar:
    st.header("研究範圍")
    market = "台股"
    st.caption("目前版本專注台股；美股資料與入口已暫時移除，以降低記憶體用量。")
    ...
    if False:  # 保留舊版研究設定供未來重新接回，但不載入美股資料。
...
try:
    with st.spinner(
        ...
        if data_mode == "Live Yahoo Finance":
            ...
            else:  # pragma: no cover - market is fixed to Taiwan in this version.
                raise RuntimeError("美股資料庫目前已停用。")
        else:
...
with tab_broker_branch:
    st.markdown("### 券商分點資金追蹤")
    st.caption(
        "追蹤各券商分公司在個股的買進、賣出與淨買超，並用分點集中度＋"
        "三大法人流向＋價格確認產生研究候選。券商分點是交易通路，不等於"
        "最終投資人身分，也不能單獨視為買進理由。"
    )
    uploaded_branch_file = st.file_uploader(
        "上傳券商分點 CSV（選用）",
        type=["csv"],
        help=(
            "可直接上傳 FinMind TaiwanStockTradingDailyReport 格式；"
            "若未上傳，系統會讀取每日更新的 broker-branches.parquet。"
        ),
    ...
    branch_institutional_labels = {
        "Daily": "最新1個交易日",
        "Weekly": "最近5個交易日",
        "Monthly": "最近20個交易日",
    }
    branch_source = f"HiStock 公開分點頁（{branch_period_labels[frequency]}）"
    if uploaded_branch_file is not None:
        try:
            branch_trades = normalize_broker_branch_trades(
                pd.read_csv(uploaded_branch_file)
            )
            branch_source = "本次上傳 CSV"
        except (ValueError, pd.errors.ParserError) as exc:
            st.error(f"券商分點 CSV 無法讀取：{exc}")
            branch_trades = pd.DataFrame()

    if not branch_trades.empty:
        branch_trades = branch_trades[
            branch_trades["Horizon"] == frequency
        ].copy()

    if branch_trades.empty:
        st.info(
            f"{frequency_settings['label']}券商分點資料庫尚未建立；每日更新會先挑出"
            "相同期間法人淨流入前 10 檔股票，再逐檔抓取公開券商分點。"
            "你也可以先上傳券商軟體匯出的 CSV。"
        )
        sample_branch_csv = pd.DataFrame(
            {
                ...
                "stock_id": ["2330", "2330"],
                "securities_trader_id": ["1020", "1021"],
                "securities_trader": ["券商分點甲", "券商分點乙"],
                "price": [1000.0, 1000.0],
                "buy": [100000, 20000],
                "sell": [10000, 90000],
            }
        ...
        st.download_button(
            "下載券商分點 CSV 範例",
            sample_branch_csv.to_csv(index=False).encode("utf-8-sig"),
        ...
        st.info(
            f"本頁股票範圍＝{branch_institutional_labels[frequency]}"
            "三大法人估算淨流入前 10 名；"
            f"每檔券商分點＝HiStock 公開頁的{branch_period_labels[frequency]}買賣超。"
        )
        branch_activity, branch_stocks = aggregate_branch_activity(
            branch_trades,
        ...
        broker_candidates = build_broker_research_candidates(
            branch_stocks,
            branch_institutional,
            taiwan_master,
        )
        branch_date = pd.to_datetime(branch_trades["Date"]).max()
        branch_metrics = st.columns(4)
        branch_metrics[0].metric("資料來源", branch_source)
        branch_metrics[1].metric("資料截止", f"{branch_date:%Y-%m-%d}")
        ...
        st.markdown(
            f"#### {frequency_settings['label']}法人流入前 10 檔：分點訊號推薦"
        )
        st.caption(
            "分數＝分點集中訊號 50%＋三大法人確認 30%＋價格確認 20%。"
            "『優先研究』不是自動下單建議，仍需檢查事件、流動性與風險。"
        )
        candidate_view = broker_candidates.head(20).copy()
        candidate_view["最大買超（億）"] = candidate_view["Top buyer value"] / 1e8
        candidate_view["法人流向（億）"] = candidate_view["Selected net value"] / 1e8
        st.dataframe(
            candidate_view[
                [
                    "Ticker",
        ...
        broker_totals = (
            branch_activity.groupby(["Broker ID", "Broker"], as_index=False)["Net value"]
            .sum()
            .sort_values("Net value", ascending=False)
        )
        top_brokers = pd.concat(
            [broker_totals.head(10), broker_totals.tail(10)],
            ignore_index=True,
        ).drop_duplicates("Broker ID")
        with chart_columns[0]:
            ...
            st.plotly_chart(
                px.bar(
                    ...
                    orientation="h",
                    color="Net value",
                    color_continuous_scale=["#ef4444", "#64748b", "#22c55e"],
                    labels={"Net value": "估算淨買超金額", "Broker": "券商分點"},
                ).update_layout(height=560, coloraxis_showscale=False),
                width="stretch",
            )
        with chart_columns[1]:
            st.markdown("#### 候選股：集中度與法人確認")
            st.plotly_chart(
                px.scatter(
                    broker_candidates.head(40),
                    x="Branch concentration score",
                    y="Selected net value",
        ...
        stock_options = {
            f"{row['Ticker']}｜{row.get('Name', '')}": row["Ticker"]
            for _, row in broker_candidates.iterrows()
        }
        selected_stock_label = st.selectbox(
            "查看個股分點明細",
            list(stock_options),
            key="broker_branch_stock_detail",
        )
        selected_stock = stock_options[selected_stock_label]
        detail = branch_activity[branch_activity["Ticker"] == selected_stock].copy()
        detail["淨買超（張）"] = detail["Net shares"] / 1000
        detail["估算淨買超（億）"] = detail["Net value"] / 1e8
        st.dataframe(
            detail[
        ...
        st.info(
            "分點淨買超金額以成交價乘淨買股數估算；同一股票全市場的買進與賣出"
            "必然互相抵銷，因此本頁研究的是買盤集中在哪些分點，而不是宣稱資金憑空流入股票。"
        )
